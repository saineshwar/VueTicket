...
namespace TicketCore.Web.Areas.Administrator.Controllers
{
    ...
    public class MyTicketController : Controller
    {
        ...
        public MyTicketController(ITicketQueries iTicketQueries,
            ...
            ITicketHistoryCommand ticketHistoryCommand, IUserMasterQueries userMasterQueries,
            IGenerateTicketNo generateTicket, IMapper mapper, IApplicationMailingService applicationMailingService)
        {
            ...
            int? searchIn, int? priorityId, long? agentsId, int? page = 1)
        {
            ...
            {
                var defaultdepartment = _departmentQueries.GetAssignedDepartmentsofAdministrator(userId).FirstOrDefault()?.Value;
                departmentId = Convert.ToInt32(defaultdepartment);
            }

            var usergridview = new AdministratorTicketGrid();
            usergridview.ListofDepartment = _departmentQueries.GetAssignedDepartmentsofAdministrator(userId);
            ...
            totalticketCount =
                _iTicketQueries.GetAdministratorEndTicketsCount(agentsId, search, statusId, searchIn, departmentId,
                    priorityId);

            var ticketlist = _iTicketQueries
                .GetAdministratorEndTicketList(agentsId, page, pageSize, departmentId, search, searchIn, statusId,
                    priorityId)
                .ToList();
        ...
        [HttpGet]
        public IActionResult AllClosedTickets(int? departmentId, string search, int? statusId,
         int? searchIn, int? priorityId, long? agentsId, int? page = 1)
        {
            if (searchIn == 1 && !string.IsNullOrEmpty(search))
            {
            ...
            totalticketCount =
                _iTicketQueries.GetAllClosedAdministratorEndTicketsCount(agentsId, search, statusId, searchIn, departmentId,
                    priorityId);

            var ticketlist = _iTicketQueries
                .GetAllClosedAdministratorEndTicketList(agentsId, page, pageSize, departmentId, search, searchIn, statusId,
                    priorityId)
                .ToList();
        ...
        public ActionResult Edit(long? ticketId)
        {
            try
            {
                ...
                ticketsViewModel.ListofDepartment = _departmentQueries.GetAssignedDepartmentsofAdministrator(userId);
                ticketsViewModel.ListofPriority = _iPriorityQueries.GetAllPrioritySelectListItem();
        ...
        public IActionResult GetAllAgentandAdminList(UserRequestViewModel userRequestViewModel)
        {
            if (userRequestViewModel.DepartmentId == null)
            {
                var list = new List<SelectListItem>();
            ...
            var agentlist = _iUserMasterQueries.GetListofAgentandAgentsAdmin(userRequestViewModel.DepartmentId);
            return Json(new { listofusers = agentlist });
        }

        [HttpGet]
        public IActionResult AllUnAssignedTickets(int? departmentId, string search, int? statusId,
          int? searchIn, int? priorityId, long? agentsId, int? page = 1)
        {
            string searchtrim = "";
            ...
            totalticketCount =
                _iTicketQueries.GetAllUnAssignedTicketCount(searchtrim, statusId, searchIn, departmentId,
                    priorityId);

            var ticketlist = _iTicketQueries
                .GetAllUnAssignedTicketList(page, pageSize, departmentId, searchtrim, searchIn, statusId,
                    priorityId)
                .ToList();
        ...
        public async Task<IActionResult> AssignUser(long? aId, long? tId)
        {
            if (aId != null && tId != null)
            {
                var getticketdetails = _iTicketQueries.GetTicketbyTicketId(tId);
                if (getticketdetails.DepartmentId != aId)
                {
                    var userId = HttpContext.Session.GetSession<long>(AllSessionKeys.UserId); 
                    var request = new RequestManualAssignViewModel()
                    {
                        TicketId = tId,
                        AssignedtoUserId = aId,
                        CreatedUserId = getticketdetails.CreatedBy,
                        DepartmentId = getticketdetails.DepartmentId,
                        PriorityId = getticketdetails.PriorityId,
                        AssignedbyUserId = userId
                    };

                    await _iTicketCommand.AssignTicketManually(request);
